...
test('Deve inserir usuário com sucesso', async () => {
  const res = await request(app).post(MAIN_ROUTE).send({ name: 'Walter White', email, passwd: '123456' })
  .set('authorization', `bearer ${user.token}`)
...
test('Não deve inserir usuário sem nome', () => {
  return request(app).post(MAIN_ROUTE).send({ email:'walter@gmail.com', passwd: '123456' })
  .set('authorization', `bearer ${user.token}`)
  .then((res) => {
...
test('Não deve inserir usuário sem email', async () => {
  const result = await request(app).post(MAIN_ROUTE).send({name: 'Jesse Pinkman', passwd: '123456'})
  .set('authorization', `bearer ${user.token}`)
...
test('Não deve inserir usuário sem senha', async (done) => {
  const result = await request(app).post(MAIN_ROUTE).send({ name: 'Walter White', email: 'walter@gmail.com' })
  .set('authorization', `bearer ${user.token}`)
  expect(result.status).toBe(400)
